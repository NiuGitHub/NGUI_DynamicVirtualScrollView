...
public partial class DynamicVirtualScrollView : UIScrollView
{
    ...
    private Type m_ViewCellClass;
    private ConstructorInfo m_ViewCellCtor;
    ...
    public void InitViewCell<T>() where T : DynamicVirtualScrollViewCell
    {
        InitViewCell(typeof(T));
        onMomentumMove -= UpdateShowView;
        onMomentumMove += UpdateShowView;
    }
    public void InitViewCell(Type t)
    {
        m_ViewCellClass = t;
        m_ViewCellCtor = m_ViewCellClass.GetConstructor(VIEW_CTOR_TYPES);
    ...
    public DynamicVirtualScrollViewCell CreateCell()
    {
        DynamicVirtualScrollViewCell item = null;
        if (m_HideList.Count < 1)
        {
            item = Activator.CreateInstance(m_ViewCellClass, new object[] { ClonePrefabAndInit() }) as DynamicVirtualScrollViewCell;
            item.SetClickCallBackFunc(OnCellClick);
        }
        else
        {
            item = m_HideList[0];
    ...
    public void ReturnCell(DynamicVirtualScrollViewCell item)
    {
